...
ZETA = zeros(1,G);
THETA = zeros(G,2);
Kp = 0.90; Ki = 0.003; Kd = 0.001; bias = 0;
...
%     end
    ...
    theta1 = atan((lat2-lat1)/(lon2-lon1));
    THETA(p,1) = theta1;
    theta2 = atan((latH-lat1)/(lonH-lon1));
    THETA(p,2) = theta2;
    if theta1-theta2 > 0
        fprintf('Turn Right \n')
    ...
    if abs(latH-lat1) < 0.001 || abs(lonH-lon1) < 0.002
        fprintf('Target Aquired: ')
        if abs(latH-lat1) < 0.001
